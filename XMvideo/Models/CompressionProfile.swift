...
enum CompressionProfile: String, CaseIterable, Identifiable, Codable {
    ...
    var parameters: CompressionParameters {
        ...
        case .custom:
            return CompressionParameters.loadFromUserDefaults()
        }
    }
...
struct CompressionParameters: Codable {
    ...
    static func loadFromUserDefaults() -> CompressionParameters {
        if let data = UserDefaults.standard.data(forKey: "customCompressionParameters"),
           let params = try? JSONDecoder().decode(CompressionParameters.self, from: data) {
            return params
        }
        // Return default if custom not found
        return CompressionParameters(crf: 23, preset: "medium", gop: 250, bframes: 3, refs: 3)
    }
    
    func saveToUserDefaults() {
        if let data = try? JSONEncoder().encode(self) {
            UserDefaults.standard.set(data, forKey: "customCompressionParameters")
        }
    }
}
